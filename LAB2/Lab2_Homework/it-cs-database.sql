...
SELECT HoTen, Email FROM ChuyenGia
WHERE SUBSTRING(HoTen, LEN(HoTen) - CHARINDEX(' ', REVERSE(HoTen)) + 2, LEN(HoTen)) LIKE 'N%'
GO
...
SELECT CT.MaCongTy, TenCongTy, COUNT(DA.MaDuAn) AS SoLuongDuAn
FROM DuAn DA
...
SELECT CG.MaChuyenGia, CG.HoTen, COUNT(CG_KN.MaChuyenGia) AS SoLuongKyNang
FROM ChuyenGia_KyNang CG_KN
INNER JOIN ChuyenGia CG
ON CG_KN.MaChuyenGia = CG.MaChuyenGia
WHERE CapDo = 5
GROUP BY CG.MaChuyenGia, CG.HoTen
HAVING COUNT(CG_KN.MaKyNang) > 0
ORDER BY CG.HoTen
GO
...
SELECT CG.MaChuyenGia, CG.HoTen, AVG(CG_KN.CapDo) AS TBCapDoKyNang
FROM ChuyenGia_KyNang CG_KN
INNER JOIN ChuyenGia CG
ON CG_KN.MaChuyenGia = CG.MaChuyenGia
WHERE CG.ChuyenNganh = N'Phát triển phần mềm'
GROUP BY CG.MaChuyenGia, CG.HoTen
HAVING AVG(CG_KN.CapDo) > 3
ORDER BY TBCapDoKyNang DESC
GO
